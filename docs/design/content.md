...
> **文档性质**：本文档分为两部分 —— **MVP（当前实现）** 和 **远期目标（设计方向）**。
> MVP 部分描述当前代码库的实际状态；远期目标部分是理想化的设计方向，用于指导长期演进。
> **当前实现参考**：`docs/architecture/engineering-baseline.md` 和 `docs/architecture/game.md`

---

## Part A：MVP（当前实现）

> 技术栈：Bevy 0.19 + Lightyear 0.28 + avian3d 0.7 + 3-crate workspace

### A1. 资源系统（已实现）

| 资源类型 | 字段 | 说明 |
|---------|------|------|
| Gold | `gold` | 金币，用于国家创建和交易 |
| Food | `food` | 食物，维持生存 |
| Wood | `wood` | 木材，建筑材料 |
| Stone | `stone` | 石材，建筑材料 |
| Iron | `iron` | 铁矿，武器材料 |
| Soul | `soul` | 灵魂，建国消耗（10 soul/国），生物死亡掉落 |

**守恒机制**：全局资源池 `GlobalPool` 记录所有资源总量，每次变动有审计日志。

### A2. 生态系统（已实现）

- **Monster**：怪物王国 + 巢穴 + 个体三级结构
  - 全局上限：5 王国 / 80 巢穴 / 1500 个体
  - 死亡掉落资源，回流全局池
- **Creature**：简单动物系统
  - 生成/消亡有计数上限
  - 与怪物系统共享掉落回流逻辑
...
- `TickObserver`：每 tick 检查资源守恒、实体上限、计数回流
- 健康检查：`health.json` 记录 PASS/PARTIAL/FAIL

---

## Part B：远期目标（设计方向）

> 以下内容是理想化的系统设计，用于指导长期演进。**不是当前实现**，也不是近期计划。
> 分阶段推进策略见 `docs/plans/doc-todo.md`。

### 阶段规划

| 阶段 | 内容 | 状态 |
|------|------|------|
| Phase 1 | 无限世界 + 地形预设 | 📋 计划中 |
| Phase 2 | 商队物流 + 外交战争 | 📋 计划中 |
| Phase 3 | 以太界 + 神器 + 终局 | 📋 计划中 |
| **远期** | 元素系统 + 基因 + 记忆 + 涌现规则 | 🔬 设计探索 |
