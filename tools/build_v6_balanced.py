"""build_v6_balanced.py — v6 平衡版: 配色 1-3 色 + 比例调整 (人小, 其他大).

v5 -> v6 改动:
- 玩家 avatar 0.8m -> 0.35m (缩小 ~55%)
- 怪物 0.4m -> 0.55m (放大)
- 树 0.7m -> 1.0m
- 石头 0.18m -> 0.28m
- 花 0.5m -> 0.7m
...
def make_player_avatar_v6() -> None:
    ...
    # 整体缩小: head y=0.32, body y=0.18, leg y=0.05
    # 大圆头 (0.14 radius, 缩小)
    uv_sphere("head", (0.0, 0.32, 0.0), (0.14, 0.13, 0.13), skin, 12, 9)
    # 头发 (0.14 radius, y=0.36)
...
def make_tree_v6() -> None:
    ...
    # 圆叶 (Q 版, 放大: 0.50 radius, y 中心 0.70)
...
def make_berry_bush_v6() -> None:
    ...
    # 散开 7 颗浆果 (距离加大到 0.5-0.85m, y 0.36~0.62 高度, 放大)
    fruit_positions = [
        (-0.55, 0.42,  0.12, 0),
        ( 0.60, 0.48, -0.18, 1),
        ( 0.05, 0.60,  0.60, 2),
