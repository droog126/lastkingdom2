...
use crate::simulation::{TickReport, WorldInput, step_world};
...
mod tests {
    ...
    fn fixed_authority_report_exposes_the_exact_shared_nature_step() {
        ...
        assert_eq!(report.snapshot, NatureSnapshot::from_ecology(clock.tick, &eco));
        assert!(report.events.iter().any(|event| matches!(event, NatureEvent::RainFell { .. })));
    }
}
