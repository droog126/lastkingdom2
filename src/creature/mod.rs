...
pub fn spawn_creatures(
    ...
    while placed < 12 && starter_attempts < 400 {
        starter_attempts += 1;
        let x = spawn_cx + rng.gen_range(-8..9);
        let z = spawn_cz + rng.gen_range(-8..9);
    ...
    while placed < target && attempts < count * 20 {
        attempts += 1;
        let x = rng.gen_range(2..(s - 2));
        let z = rng.gen_range(2..(s - 2));
        // 离出生点 8 格以外（避免覆盖起始牧场）
...
fn try_spawn_creature(
    ...
    commands.spawn((
        ...
        CreatureAI {
            wander_timer: 0.0,
            next_wander_secs: rng.gen_range(0.5..1.5),  // 起始牧场动得快点
...
pub fn update_creatures(
    ...
    for (mut creature, mut ai, mut tf) in q.iter_mut() {
        ...
        ai.wander_timer = 0.0;
        ai.next_wander_secs = rng.gen_range(1.5..3.0);
