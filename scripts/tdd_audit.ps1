...
function Test-DecisionTemplateSync {
    $loopPath = Join-Path $ProjectRoot "loop.ps1"
    $agentPath = Join-Path $ProjectRoot "Agent.md"

    if (-not (Test-Path $loopPath) -or -not (Test-Path $agentPath)) {
        Write-Host "decision template audit skipped: loop.ps1 or Agent.md missing" -ForegroundColor Yellow
        return
    }
    ...
    if ($missing.Count -gt 0) {
        ...
        foreach ($fragment in $missing) {
            Write-Host ("- missing in loop.ps1: {0}" -f $fragment)
        }
        exit 1
    }
    ...
    if (-not ($loopContent -match '# \$latestIterName decision')) {
        Write-Host ""
        Write-Host "## Decision template drift" -ForegroundColor Red
        Write-Host "- missing dynamic decision heading in loop.ps1"
        exit 1
    }
    ...
    Write-Host "- loop.ps1 decision template matches Agent.md required fields"
}
