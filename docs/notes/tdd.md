...
- 默认编译面的每个 `.rs` 文件有多少真实 `#[test]` 行
- 哪些默认模块没有直接单元测试
- 哪些模块被 `experimental-gameplay` 隔离，不计入默认测试总数
- 哪些顶层模块只在自身实现/测试中出现，没有外部运行时引用
- 当前默认 core 测试总数与 experimental 测试总数

审计不是覆盖率报告，也不能代替服务端/客户端集成测试；它用于发现“完全没测试”、
“只有测试引用”和“实验骨架被误算成上线能力”三类问题。新增核心模块时，至少要让
audit 不出现未经解释的新空白或无运行时引用模块。
...
- [x] `protection`：中途加入保护期玩家不能攻击，也不能被攻击；服务端权威伤害链路同时校验 attacker/target。
