...
> **版本**: v1.2
> **日期**: 2026-07-06
> **状态**: 冻结的历史计划；其中测试数量、世界尺寸和实现状态不再作为当前事实
...
> 2026-07-11 更新：本节的逐模块数字是历史快照，不再作为当前测试总数事实源。
> 当前默认/experimental 测试数、无测试模块和无运行时引用模块以 `just audit-tdd` 为准。
> `equipment`、`mining_site`、`terrain_overlay`、`sovereign_spark` 属于
> `lk2-core/experimental-gameplay`，在完整接入运行时前不计入默认测试覆盖。

### 3.1 当前测试状态

| 层级 | 位置 | 状态 | 测试数 |
|------|------|------|--------|
| 单元规则 | `crates/core/src/**` | ✅ 默认面持续审计 | 运行 `just audit-tdd` |
| crate 集成 | `crates/client` / `crates/server` | ⚠️ 仍需加强 | 运行分 crate 测试 |
