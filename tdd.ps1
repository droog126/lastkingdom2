...
switch ($Scope) {
    ...
    "audit" {
        & (Join-Path $ProjectRoot "scripts\tdd_audit.ps1")
        if (-not $?) {
    ...
    "changed" {
        ...
        $touchesCore = $changed | Where-Object { $_ -like "crates/core/*" }
        $touchesClient = $changed | Where-Object { $_ -like "crates/client/*" }
        $touchesServer = $changed | Where-Object { $_ -like "crates/server/*" }
