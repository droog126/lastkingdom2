...
> 当前说明：这是 2026-06-07 的拆分计划，**已完成**。仓库现在是 `lk2-core` / `lk2-client` / `lk2-server` workspace，工程边界以 `docs/architecture/engineering-baseline.md` 为准；闭环入口以 `docs/STARTING.md` 和 `.codex/skills/closed-loop-ai-dev/SKILL.md` 为准。文中 `loop.ps1`、`.harness/reins` 和 `scripts/` 工作流描述按历史背景阅读。

> 日期: 2026-06-07
> 状态: ✅ **已完成**（2026-07-06 更新）
> 范围: 仓库从「单 binary 单 crate」拆成「workspace 3 crate」 — `lk2-core` (sim 逻辑) / `lk2-server` (headless MinimalPlugins) / `lk2-client` (DefaultPlugins)
> 当前技术栈: **Bevy 0.19 + Lightyear 0.28 + Avian3D 0.7**（原计划为 Bevy 0.18 + Lightyear 0.26）
...
**关键发现**（盘点现状后）:
- `Cargo.toml` 已经升级到 **bevy 0.19 + lightyear 0.28 + leafwing-input-manager 0.21 + avian3d 0.7**
- `pvp::systems_server.rs` 和 `pvp::systems_client.rs` **已经按 client/server 拆过**（雏形）
- `network::protocol.rs` 已经写了 message / component 定义 — ✅ **已接 lightyear 0.28 的注册 API**
- `controller/components.rs` 注释: "所有状态都是 ECS 组件，天然支持 lightyear 网络同步"
- ✅ **项目已拆成 3-crate workspace** — `lk2-core` / `lk2-server` / `lk2-client`
- 也就是说: **计划已完成, 工具链已到位**。
